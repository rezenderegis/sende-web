...
import {
  ...
  ChevronLeft, ChevronRight, Play, Users, PhoneCall, BotMessageSquare, ExternalLink,
} from 'lucide-react'
...
import type {
  ...
  UpcomingDispatch, AutomationStats, AutomationAudience, CampaignPrompt,
} from '@/types'
...
function RuleForm({
  ...
  const [campaignPromptId, setCampaignPromptId] = useState<string | null>(initial?.campaignPromptId ?? null)
  ...
  const selectedPrompt = campaignPrompts.find((p) => p.id === campaignPromptId) ?? null
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Prompt picker modal */}
      {promptPickerOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setPromptPickerOpen(false)}>
          <div className="w-full max-w-lg rounded-2xl border bg-white shadow-2xl flex flex-col max-h-[70vh]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="border-t px-4 py-3 flex justify-between items-center">
              <a href="/settings/prompts" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-muted-foreground hover:text-gray-900">
                <ExternalLink className="h-3 w-3" />Gerenciar biblioteca
              </a>
              <Button variant="outline" size="sm" onClick={() => setPromptPickerOpen(false)}>Cancelar</Button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="space-y-2 border-t pt-3">
        <div className="flex items-center justify-between">
          <label className="text-xs font-medium text-gray-700">
            Prompt de campanha <span className="text-muted-foreground font-normal">(opcional)</span>
          </label>
          <div className="flex items-center gap-3">
            <a href="/settings/prompts" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-muted-foreground hover:text-gray-700">
        ...
        <p className="text-xs text-muted-foreground">
          Quando o contato responder, o bot usará este prompt por 72h.
        </p>
      </div>
