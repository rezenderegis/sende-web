...
import type { Lead, LeadBrand } from '@/types'

const brandConfig: Record<LeadBrand, { label: string; badge: string }> = {
  sende: { label: 'Sende', badge: 'bg-teal-100 text-teal-700' },
  globalsix: { label: 'GlobalSix', badge: 'bg-[#8257E5]/10 text-[#8257E5]' },
}
  ...
  return (
    <div className="group flex items-center gap-4 px-4 py-3.5 hover:bg-teal-50/40 transition-colors border-b border-[#EEF2F6] last:border-0">
      ...
      <div className="w-24 shrink-0">
        <span className={cn('inline-flex items-center rounded-pill px-2.5 py-1 text-xs font-medium', brandConfig[lead.brand].badge)}>
          {brandConfig[lead.brand].label}
        </span>
      </div>
...
export default function LeadsPage() {
  ...
  const [brandFilter, setBrandFilter] = useState<'all' | LeadBrand>('all')
      ...
      <div className="border-b border-[#EEF2F6] bg-white px-6 py-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-ink-faint">Contatos que se cadastraram pelo formulário do site (Sende e GlobalSix)</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-2">
            {(['all', 'sende', 'globalsix'] as const).map((b) => (
              <button
                key={b}
              ...
              >
                {b === 'all' ? 'Todos' : brandConfig[b].label}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-card border border-[#EEF2F6] bg-white shadow-sende-sm overflow-hidden">
          <div className="hidden md:flex items-center gap-4 border-b border-[#EEF2F6] bg-gray-50/60 px-4 py-2.5">
            ...
            <div className="w-24 shrink-0 text-xs font-semibold uppercase tracking-wider text-ink-faint">Marca</div>
